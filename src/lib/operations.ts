...
export function validateLoad(draft:LoadDraft){const errors:Record<string,string>={};for(const key of ["origin","destination","customer","driver","unit"] as const)if(!draft[key].trim())errors[key]=`${key[0].toUpperCase()+key.slice(1)} is required.`;if(!Number.isFinite(draft.rate)||draft.rate<=0)errors.rate="Rate must be greater than zero.";if(!Number.isFinite(draft.plannedMiles)||draft.plannedMiles<=0)errors.plannedMiles="Miles must be greater than zero.";if(!draft.pickupOn)errors.pickupOn="Pickup date is required.";if(!draft.deliveryOn)errors.deliveryOn="Delivery date is required.";if(draft.pickupOn&&draft.deliveryOn&&draft.deliveryOn<draft.pickupOn)errors.deliveryOn="Delivery cannot be before pickup.";return errors}
