...
const ink=rgb(.05,.18,.23),teal=rgb(.07,.48,.43),muted=rgb(.37,.43,.47);
...
export async function renderInvoicePdf(invoice:InvoiceDocument):Promise<Uint8Array> {
 ...
 function newPage(){page=pdf.addPage([612,792]);y=746;draw(invoice.number,44,y,13,teal);draw(invoice.status==="draft"?"DRAFT — NOT ISSUED":"INVOICE",410,y,12,teal);y-=32}
 ...
 newPage();
 paragraph(invoice.company.name,18);if(invoice.company.displayName&&invoice.company.displayName!==invoice.company.name)paragraph(invoice.company.displayName,10,524,muted);
 ...
 pdf.setTitle(`${invoice.number} — ${invoice.customer.name}`);pdf.setAuthor(invoice.company.name);pdf.setCreator("King’s Accounting");
