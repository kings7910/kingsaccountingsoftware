export type FuelEntry={accountingPosted?:boolean;receiptCount?:number;id:string;date:string;unit:string;vendor:string;location:string;gallons:number;totalCost:number;odometer:number;state:string;receiptReference:string};
export type FuelDraft=Omit<FuelEntry,"accountingPosted"|"receiptCount"|"id">;
