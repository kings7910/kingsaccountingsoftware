...
export function validateFuelEntry(draft:FuelDraft){const errors:Record<string,string>={};if(!draft.date)errors.date="Date is required.";if(!draft.unit.trim())errors.unit="Unit is required.";if(!draft.vendor.trim())errors.vendor="Vendor is required.";if(draft.gallons<=0)errors.gallons="Gallons must be greater than zero.";if(draft.totalCost<=0)errors.totalCost="Cost must be greater than zero.";if(draft.odometer<0)errors.odometer="Odometer cannot be negative.";return errors}
