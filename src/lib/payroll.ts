export const payStatuses=["Draft","Approved","Paid"] as const;
export type PayStatus=(typeof payStatuses)[number];
export type Settlement={id:string;driver:string;periodStart:string;periodEnd:string;loads:number;grossPay:number;reimbursements:number;deductions:number;status:PayStatus;paidDate:string};
...
export function validateSettlement(item:SettlementDraft){const errors:Record<string,string>={};if(!item.driver.trim())errors.driver="Driver is required.";if(!item.periodStart||!item.periodEnd)errors.period="Pay period is required.";else if(item.periodEnd<item.periodStart)errors.period="End date must follow start date.";if(item.loads<0||item.grossPay<0||item.reimbursements<0||item.deductions<0)errors.amount="Amounts cannot be negative.";if(netPay(item)<0)errors.amount="Deductions cannot exceed earnings.";if(item.status==="Paid"&&!item.paidDate)errors.paidDate="Paid date is required.";return errors}
export function nextPayStatus(status:PayStatus):PayStatus|null{return status==="Draft"?"Approved":status==="Approved"?"Paid":null}
export function payrollSummary(items:Settlement[]){return{draft:items.filter(x=>x.status==="Draft").length,approved:items.filter(x=>x.status==="Approved").length,paid:items.filter(x=>x.status==="Paid").reduce((sum,x)=>sum+netPay(x),0),payable:items.filter(x=>x.status!=="Paid").reduce((sum,x)=>sum+netPay(x),0)}}
