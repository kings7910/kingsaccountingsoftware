...
export async function listNotifications(companyId:string):Promise<WorkspaceNotification[]>{const{supabase,userId}=await authenticatedClient(),{data,error}=await supabase.from("notifications").select("id,title,body,kind,read_at,created_at").eq("company_id",companyId).eq("user_id",userId).order("created_at",{ascending:false}).limit(20);if(error)throw new Error(error.message);return(data??[]).map(item=>({id:item.id,title:item.title,body:item.body,kind:item.kind,read:Boolean(item.read_at),createdAt:item.created_at}))}
