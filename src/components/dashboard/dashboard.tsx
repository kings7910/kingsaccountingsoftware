...
export function Dashboard({userName,onNavigate,companyId}:{userName:string;onNavigate:(value:string)=>void;companyId?:string}) {
  ...
  return <div className="mx-auto max-w-[1600px] p-4 md:p-7 xl:p-9">{error&&<div role="alert" className="mb-4 rounded-xl bg-red-50 p-3 text-sm font-bold text-red-700">{error}</div>}
    <section className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="mb-1 text-sm font-semibold text-[var(--teal)]">{today}</p><h1 className="display text-3xl font-extrabold text-[var(--navy)] md:text-4xl">{greeting}, {firstName}.</h1><p className="mt-2 text-sm text-[var(--muted)]">Month to date versus the previous full month. Totals include records your role can access.</p></div>
      <label className="relative flex w-fit items-center gap-2 rounded-xl border border-[var(--line)] bg-white px-4 py-2.5 text-sm font-bold text-[var(--navy)]"><CalendarDays size={17}/><select aria-label="Dashboard chart period" value={period} onChange={event=>setPeriod(event.target.value)} className="appearance-none bg-transparent pr-5 font-bold"><option value="1">This month</option><option value="3">Last 3 months</option><option value="6">Last 6 months</option></select><ChevronDown className="pointer-events-none absolute right-3" size={15}/></label>
    </section>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {dashboard.stats.map((stat,index)=><article className="card relative overflow-hidden p-5" key={stat.label}>
        ...
        <p className={`mt-2 flex items-center gap-1 text-xs font-bold ${index===1?"text-[#bd6b43]":"text-[var(--teal)]"}`}>{stat.change.startsWith("+")?<TrendingUp size={13}/>:stat.change.startsWith("−")?<TrendingDown size={13}/>:null} {stat.change} {/[+−]\d/.test(stat.change)&&<span className="font-medium text-[var(--muted)]">vs last month</span>}</p>
      </article>)}
    </section>

    <p className="mt-3 text-xs text-[var(--muted)]">Financial totals include posted journals only. Unposted transactions and settlements are excluded. Per-mile figures divide this month’s posted income and expenses by approved mileage; fuel spend is shown separately.</p>
    <section className="mt-4 grid gap-4 xl:grid-cols-[1.55fr_1fr]">
      <article className="card min-h-[370px] p-5 md:p-6">
        <div className="mb-4 flex items-start justify-between"><div><h2 className="display text-xl font-extrabold text-[var(--navy)]">Income and expenses</h2><p className="mt-1 text-sm text-[var(--muted)]">Posted ledger activity · selected months through today</p></div><div className="flex gap-4 text-xs font-semibold"><span className="flex items-center gap-1.5"><i className="size-2 rounded-full bg-[var(--teal)]"/>Income</span><span className="flex items-center gap-1.5"><i className="size-2 rounded-full bg-[#c9d5d9]"/>Expenses</span></div></div>
      ...
      <article className="card p-5 md:p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[{l:"Total mileage",v:dashboard.fleet.totalMileage.toLocaleString(),s:"miles",i:Gauge},{l:"Loaded mileage",v:dashboard.fleet.loadedMileage.toLocaleString(),s:dashboard.fleet.totalMileage?`${(dashboard.fleet.loadedMileage/dashboard.fleet.totalMileage*100).toFixed(1)}% loaded`:"0% loaded",i:Truck},{l:"Revenue / mile",v:money.format(dashboard.fleet.revenuePerMile),s:"per mile",i:TrendingUp},{l:"Cost / mile",v:money.format(dashboard.fleet.costPerMile),s:"per mile",i:TrendingDown}].map(({l,v,s,i:Icon})=><div key={l} className="rounded-2xl bg-[var(--canvas)] p-4"><Icon size={18} className="mb-4 text-[var(--teal)]"/><p className="text-xs font-semibold text-[var(--muted)]">{l}</p><p className="display mt-1 text-xl font-extrabold text-[var(--navy)]">{v}</p><p className="mt-1 text-xs font-bold text-[var(--teal)]">{s}</p></div>)}
        </div>
        <div className="mt-4 rounded-2xl border border-[#f0dfb6] bg-[#fffaf0] p-4"><div className="flex items-center justify-between text-sm"><span className="flex items-center gap-2 font-bold text-[var(--navy)]"><Fuel size={17} className="text-[var(--gold)]"/>Fuel cost</span><strong>{money.format(dashboard.fleet.fuelCost)}</strong></div><p className="mt-2 text-xs text-[var(--muted)]">{money.format(dashboard.fleet.fuelPerMile)} per mile from recorded fuel and mileage</p></div>
      </article>
    </section>
