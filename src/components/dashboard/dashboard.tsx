...
import { alerts, cashFlow, loads, recent, stats } from "@/lib/demo-data";
...
export function Dashboard({onNavigate}:{onNavigate:(value:string)=>void}) {
  return <div className="mx-auto max-w-[1600px] p-4 md:p-7 xl:p-9">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {stats.map((stat,index)=><article className="card relative overflow-hidden p-5" key={stat.label}>
    ...
    <section className="mt-4 grid gap-4 xl:grid-cols-[1.55fr_1fr]">
      ...
      <article className="card p-5 md:p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[{l:"Total mileage",v:"42,816",s:"miles",i:Gauge},{l:"Loaded mileage",v:"35,740",s:"83.5%",i:Truck},{l:"Revenue / mile",v:"$4.32",s:"+$0.28",i:TrendingUp},{l:"Cost / mile",v:"$2.79",s:"−$0.11",i:TrendingDown}].map(({l,v,s,i:Icon})=><div key={l} className="rounded-2xl bg-[var(--canvas)] p-4"><Icon size={18} className="mb-4 text-[var(--teal)]"/><p className="text-xs font-semibold text-[var(--muted)]">{l}</p><p className="display mt-1 text-xl font-extrabold text-[var(--navy)]">{v}</p><p className="mt-1 text-xs font-bold text-[var(--teal)]">{s}</p></div>)}
        </div>
        <div className="mt-4 rounded-2xl border border-[#f0dfb6] bg-[#fffaf0] p-4"><div className="flex items-center justify-between text-sm"><span className="flex items-center gap-2 font-bold text-[var(--navy)]"><Fuel size={17} className="text-[var(--gold)]"/>Fuel cost</span><strong>$31,284</strong></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#eee4ce]"><div className="h-full w-[68%] rounded-full bg-[var(--gold)]"/></div><p className="mt-2 text-xs text-[var(--muted)]">$0.73 per mile · 68% of budget</p></div>
      </article>
    </section>
    ...
    <section className="mt-4 grid gap-4 xl:grid-cols-[1.45fr_1fr]">
      <article className="card overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left"><thead><tr className="border-y border-[var(--line)] bg-[var(--canvas)] text-[11px] uppercase tracking-wider text-[var(--muted)]"><th className="px-6 py-3">Load</th><th>Lane</th><th>Driver / unit</th><th>Revenue</th><th className="pr-6">Status</th></tr></thead><tbody>{loads.map(load=><tr key={load.number} className="border-b border-[var(--line)] last:border-0"><td className="px-6 py-4 text-sm font-extrabold text-[var(--navy)]">{load.number}</td><td className="py-4 text-sm font-semibold">{load.lane}</td><td className="py-4"><p className="text-sm font-semibold">{load.driver}</p><p className="text-xs text-[var(--muted)]">Unit {load.unit}</p></td><td className="py-4 text-sm font-bold">{load.value}</td><td className="pr-6"><span className={`pill ${load.status==="Delivered"?"bg-[#e8f5ee] text-[#287250]":"bg-[#e5f2f5] text-[#23657a]"}`}>{load.status}</span></td></tr>)}</tbody></table></div>
      </article>
      <article className="card p-5 md:p-6"><div className="mb-4 flex items-center justify-between"><div><h2 className="display text-xl font-extrabold text-[var(--navy)]">Needs attention</h2><p className="mt-1 text-sm text-[var(--muted)]">Items that could use a look</p></div><CircleAlert className="text-[#d97b47]"/></div><div className="space-y-2">{alerts.map((alert,index)=><button key={alert.title} className="flex w-full items-center gap-3 rounded-xl p-3 text-left hover:bg-[var(--canvas)]"><div className={`grid size-10 shrink-0 place-items-center rounded-xl ${index===0?"bg-[#fff0e8] text-[#c96535]":index===1?"bg-[var(--teal-light)] text-[var(--teal)]":"bg-[#fff8e4] text-[#aa7720]"}`}>{index===0?<FileWarning size={18}/>:index===1?<Wrench size={18}/>:<Clock3 size={18}/>}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{alert.title}</p><p className="text-xs text-[var(--muted)]">{alert.detail}</p></div><span className="text-[11px] font-bold text-[var(--muted)]">{alert.urgency}</span></button>)}</div></article>
    </section>

    <article className="card mt-4 overflow-hidden"><div className="flex items-center justify-between p-5 pb-3 md:px-6"><div><h2 className="display text-xl font-extrabold text-[var(--navy)]">Recent activity</h2><p className="mt-1 text-sm text-[var(--muted)]">Latest money movement</p></div><button onClick={()=>onNavigate("Transactions")} className="flex items-center gap-1 text-sm font-bold text-[var(--teal)]">All transactions <ArrowRight size={16}/></button></div><div className="grid divide-y divide-[var(--line)] border-t border-[var(--line)] md:grid-cols-2 md:divide-x md:divide-y-0">{recent.map((item,index)=><div key={item.detail} className={`flex items-center gap-3 p-4 md:px-6 ${index>1?"md:hidden xl:flex":""}`}><div className={`grid size-10 shrink-0 place-items-center rounded-xl ${item.kind==="income"?"bg-[var(--teal-light)] text-[var(--teal)]":"bg-[var(--canvas)] text-[var(--navy)]"}`}><Receipt size={17}/></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{item.title}</p><p className="text-xs text-[var(--muted)]">{item.detail}</p></div><div className="text-right"><p className={`text-sm font-extrabold ${item.kind==="income"?"text-[var(--teal)]":"text-[var(--navy)]"}`}>{item.amount}</p><p className="text-[11px] font-semibold text-[var(--muted)]">{item.status}</p></div></div>)}</div></article>
  </div>;
}
