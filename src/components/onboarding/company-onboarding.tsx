  ...
  return <main className="min-h-screen bg-[var(--navy)] p-4 sm:grid sm:place-items-center">
    <div className="grid w-full max-w-5xl overflow-hidden rounded-3xl bg-white shadow-2xl lg:grid-cols-[.9fr_1.1fr]">
      ...
      <section className="p-7 sm:p-12"><div className="grid size-12 place-items-center rounded-2xl bg-[var(--teal-light)] text-[var(--teal)]"><Building2/></div><p className="mt-6 text-sm font-bold text-[var(--teal)]">Welcome, {userName}</p><h2 className="display mt-2 text-3xl font-extrabold text-[var(--navy)]">Set up your company</h2><p className="mt-2 text-sm text-[var(--muted)]">Enter the business name your team should see.</p>
        <form onSubmit={submit} className="mt-8 space-y-5"><label className="block text-sm font-bold">Company display name<input name="displayName" required minLength={2} maxLength={120} autoFocus className="mt-2 w-full rounded-xl border border-[var(--line)] p-3" placeholder="King’s Transport LLC"/></label><label className="block text-sm font-bold">Legal business name <span className="font-normal text-[var(--muted)]">(optional)</span><input name="legalName" maxLength={180} className="mt-2 w-full rounded-xl border border-[var(--line)] p-3" placeholder="Defaults to the display name"/></label>{error&&<p role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800">{error}</p>}<button disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--teal)] p-3.5 font-bold text-white disabled:opacity-50">{busy?"Creating workspace…":"Create workspace"}<ArrowRight size={18}/></button></form>
      </section>
    </div>
  </main>;
}
