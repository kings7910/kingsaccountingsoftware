...
export function FuelReceipts({entryId,onUploaded}:{entryId:string;onUploaded?:()=>void}){
  ...
  async function attach(file:File){
    ...
    try{const form=new FormData();form.set("file",file);await uploadFuelReceipt(entryId,form);setMessage("Receipt saved and connected to this fuel entry.");setReload(x=>x+1);onUploaded?.()}
...
export function DriverFuelReceipts({loadId,refresh,onUploaded}:{loadId:string;refresh:number;onUploaded:()=>void}){
  ...
  return <article className="card p-5"><h2 className="display text-xl font-extrabold">Fuel receipts</h2>
    ...
    {error&&<p role="alert" className="mt-3 text-sm text-red-700">{error}</p>}
    {loading?<p role="status" className="mt-3 text-sm">Loading fuel entries…</p>:entries.length?<><label className="mt-4 block text-xs font-bold">Fuel entry<select className="input" value={selected} onChange={e=>setSelected(e.target.value)}>{entries.map(entry=><option key={entry.id} value={entry.id}>{entry.label}</option>)}</select></label><div className="mt-4"><FuelReceipts key={selected} entryId={selected} onUploaded={onUploaded}/></div><p className="mt-3 text-xs text-[var(--muted)]">Showing the 50 most recent fuel entries for this load.</p></>:<p className="mt-3 text-sm">No submitted fuel entries for this load yet.</p>}
